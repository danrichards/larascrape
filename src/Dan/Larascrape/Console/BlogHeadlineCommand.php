 ...
 * @todo Needs rewrite.
 */
class BlogHeadlineCommand extends Command {

    protected $signature = "crawl:blog:headline
        {url : A url to scrape.}
        ...
        try {
            ...
            if ($headlines->count()) {
                if ($top) {
                    ...
                    $name = $first->nodeName();
                    $score = number_format($first->getScoreTotal("headline"), 1);
                    ...
                    foreach ($headlines as $h) {
                        $name = $h->nodeName();
                        $score = number_format($h->getScoreTotal("headline"), 1);
